...
<!-- Julia simulation omitted; using pre-generated image -->

::: {#fig-ols fig-cap="OLS $\hat{\phi}$ distributions for stationary AR(1) and random walk processes"}
![](properties_of_time_series_new_files/ols_estimator_distributions.pdf){width=90%}
:::
...
if (file.exists("sim_run_1e6.log")) {
  ...
  if (!requireNamespace("kableExtra", quietly = TRUE)) {
    cat("\\nPackage 'kableExtra' not installed; printing basic table.\\n")
    names(df) <- c("Process","True $\\phi$", "Mean of $\\hat{\\phi}$", "Std of $\\hat{\\phi}$", "Bias of $\\hat{\\phi}$")
    print(kable(df, caption = paste0("Monte Carlo summary (n=", sample_n, ", reps=", format(reps, big.mark=','), ", burn-in=", burnin, ")"), digits = 4, format = "pandoc", escape = FALSE))
  } else {
    library(kableExtra)
    col_names <- c("Process", "$\\phi$", "Mean of $\\hat{\\phi}$", "Std of $\\hat{\\phi}$", "Bias of $\\hat{\\phi}$")
    tbl <- knitr::kable(df, format = "latex", booktabs = TRUE, digits = 4,
                 caption = paste0("Monte Carlo summary (n=", sample_n, ", reps=", format(reps, big.mark=','), ", burn-in=", burnin, ")"),
                 align = c("l","r","r","r","r"), col.names = col_names, escape = FALSE)
    tbl <- kable_styling(tbl, latex_options = c("hold_position", "scale_down"))
    cat(tbl)
  }
} else {
  cat("**Simulation log not found:** sim_run_1e6.log\\n")
}
```
